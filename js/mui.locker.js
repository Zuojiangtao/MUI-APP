	...
	var Locker = $.Locker = $.Class.extend({
		...
		draw: function(cxt, _PointLocationArr, _LinePointArr, touchPoint) {
			...
			if (_LinePointArr.length > 0) {
				...
				cxt.strokeStyle = self.options.lineColor || "#FFAA00"; //连结线颜色
				cxt.stroke();
				cxt.closePath();
				if (touchPoint != null) {
			...
			for (var i = 0; i < _PointLocationArr.length; i++) {
				...
				cxt.fillStyle = self.options.ringColor || "#545454"; //圆圈边框颜色
				cxt.beginPath();
				cxt.arc(Point.X, Point.Y, R, 0, Math.PI * 2, true);
				cxt.closePath();
				cxt.fill();
				cxt.fillStyle = self.options.fillColor || "#efeff4"; //圆圈填充颜色
				cxt.beginPath();
				cxt.arc(Point.X, Point.Y, R - ((self.options.ringWidth || 2) * self.options.times), 0, Math.PI * 2, true);
				cxt.closePath();
				cxt.fill();
				if (_LinePointArr.indexOf(i) >= 0) {
					cxt.fillStyle = self.options.pointColor || "#FFAA00"; //圆圈中心点颜色
					cxt.beginPath();
